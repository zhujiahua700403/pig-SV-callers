...
run_stats_dir="${BASE_DIR}/result/WGS-SVs/run/wham"  # Runtime statistics directory

# ========== Initialization ==========
mkdir -p "$run_stats_dir" "$output_base"
...
process_sample() {
    ...
    bash -c "
    # Step 1: Run WHAM
    echo \"[$(date '+%Y-%m-%d %H:%M:%S')] Running WHAM detection...\"
    whamg -x 1 \
        -f \"$input_bam\" \
        -a \"$reference_genome\" \
        -c \"$chromosomes\" \
